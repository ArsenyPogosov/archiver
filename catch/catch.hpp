...
#ifdef _MSC_VER
#pragma warning(push)
#pragma warning(disable : 4180)  // We attempt to stream a function (address) by const&, which MSVC complains about but
                                 // is harmless
#endif
...
OutlierClassification classify_outliers(ConstIterator first, ConstIterator last) {
...
template <typename ConstIterator>
double mean(ConstIterator first, ConstIterator last) {
    auto count = last - first;
    double sum = std::accumulate(first, last, 0.);
    return sum / count;
}

template <typename URng, typename ConstIterator, typename Estimator>
sample resample(URng& rng, int resamples, ConstIterator first, ConstIterator last, Estimator& estimator) {
    auto n = last - first;
...
sample jackknife(Estimator&& estimator, ConstIterator first, ConstIterator last) {
    auto n = last - first;
...
template <typename ConstIterator, typename Estimator>
Estimate<double> bootstrap(double confidence_level, ConstIterator first, ConstIterator last, sample const& resample,
                           Estimator&& estimator) {
...
namespace Benchmark {
namespace Detail {
template <typename Duration, typename ConstIterator>
SampleAnalysis<Duration> analyse(const IConfig& cfg, Environment<Duration>, ConstIterator first, ConstIterator last) {
...
public:
    ...
    TokenStream(ConstIterator it, ConstIterator itEnd) : it(it), itEnd(itEnd) {
        loadBuffer();
    }
